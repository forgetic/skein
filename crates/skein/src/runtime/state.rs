...
impl RuntimeState {
    ...
    /// Like [`create_task`](Self::create_task), but hands the task's own `Cx`
    /// to a factory closure so the spawned future can use its capability
    /// context directly instead of relying on ambient `Cx::current()`.
    ///
    /// Backs `RuntimeHandle::spawn_with_cx`.
    pub(crate) fn create_task_with_cx<F, Fut>(
        &mut self,
        region: RegionId,
        budget: Budget,
        factory: F,
    ) -> Result<(TaskId, crate::runtime::TaskHandle<()>), SpawnError>
    where
        ...
        Fut: Future<Output = ()> + Send + 'static,
    {
        ...
        let (task_id, handle, cx, result_tx) =
            self.create_task_infrastructure::<()>(region, budget)?;

        // Build the user future with the task's own Cx (cloned), keeping the
        // original to report completion so a `TaskHandle` can observe it.
        let user_future = factory(cx.clone());
        let wrapped_future = async move {
            user_future.await;
