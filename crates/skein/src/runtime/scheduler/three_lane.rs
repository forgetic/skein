    ...
    pub(crate) fn execute(&self, task_id: TaskId) {
        ...
        let (mut stored, wake_state, priority, cx_inner, cached_waker, cached_cancel_waker, task_cx) = {
            ...
            let merged = self.with_task_table(|tt| {
                ...
                // Clone the task's Cx so it can be installed as the ambient
                // context for the duration of the poll (below). Tasks observe
                // their capabilities via `Cx::current()`.
        ...
        let poll_result = {
            ...
            // Install the task's capability context as ambient for the poll, so
            // futures can reach it via `Cx::current()`.
            let _task_cx_guard = crate::cx::Cx::set_current(task_cx);
            stored.poll(&mut cx)
        };
