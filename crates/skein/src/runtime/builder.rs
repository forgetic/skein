...
impl RuntimeHandle {
    ...
    /// Spawn a task and hand its capability context to the factory closure.
    ///
    /// The factory receives a child [`Cx`](crate::cx::Cx) created in the
    /// runtime's root region — the same context the task observes while polled,
    /// which participates in structured cancellation on shutdown. Unlike a bare
    /// [`spawn`](Self::spawn), the `Cx` is established up front and passed by
    /// value, so the future need not rely on ambient `Cx::current()`.
    ///
    /// # Panics
    ///
    /// Panics if the task cannot be admitted to the root region. Use
    /// [`try_spawn_with_cx`](Self::try_spawn_with_cx) to handle that explicitly.
    ...
    /// Spawn a task with its capability context, returning admission errors
    /// instead of panicking. See [`spawn_with_cx`](Self::spawn_with_cx).
    pub fn try_spawn_with_cx<F, Fut>(&self, f: F) -> Result<(), SpawnError>
    where
        F: FnOnce(crate::cx::Cx) -> Fut + Send + 'static,
        Fut: Future<Output = ()> + Send + 'static,
    {
        self.inner.spawn_with_cx(f)
    }
...
impl RuntimeInner {
    ...
    fn spawn_with_cx<F, Fut>(&self, factory: F) -> Result<(), SpawnError>
    where
        F: FnOnce(crate::cx::Cx) -> Fut + Send + 'static,
        Fut: Future<Output = ()> + Send + 'static,
    {
        let task_id = {
            let mut guard = self.state.lock().expect("runtime state lock poisoned");
            let (task_id, _handle) =
                guard.create_task_with_cx(self.root_region, Budget::INFINITE, factory)?;
            task_id
        };
