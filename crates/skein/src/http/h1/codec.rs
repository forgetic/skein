...
fn parse_chunk_size_line(line: &[u8]) -> Result<usize, HttpError> {
    ...
    // chunk-size = 1*HEXDIG. The size must be strictly hex digits — no sign,
    // no surrounding whitespace, no `0x` prefix — otherwise a lenient parser
    // and a strict one can disagree on the body length (request smuggling).
    // The optional chunk-ext after `;` is opaque and ignored.
    ...
    let size_str = std::str::from_utf8(size_part).map_err(|_| HttpError::BadChunkedEncoding)?;
    usize::from_str_radix(size_str, 16).map_err(|_| HttpError::BadChunkedEncoding)
}

/// Parse a `Content-Length` field value.
///
/// RFC 9112 §6.2: Content-Length = 1*DIGIT. Reject anything else — a leading
/// `+`/`-`, surrounding or interior whitespace, hex, or empty — since any
/// laxity here is a request-smuggling / length-confusion vector.
...
fn decode_head(
    ...
    let kind = match (transfer_encoding_idx, content_length_idx) {
        ...
        (None, Some(cl_idx)) => {
            let len = parse_content_length(headers[cl_idx].1.as_str())?;
