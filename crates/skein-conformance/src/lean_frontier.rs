...
mod tests {
    ...
    fn parse_diagnostic_line_extracts_expected_fields() {
        let line = "error: Skein.lean:2874:2: Alternative `cancelChild` has not been provided";
        let diagnostic = parse_diagnostic_line(line).expect("must parse");
        assert_eq!(diagnostic.severity, LeanDiagnosticSeverity::Error);
        assert_eq!(diagnostic.file_path, "Skein.lean");
    ...
    fn extraction_is_deterministic_for_same_input() {
        let log = "\
error: Skein.lean:10:2: Unknown identifier `x`\n\
error: Skein.lean:11:2: Unknown identifier `y`\n\
error: Skein.lean:12:3: Type mismatch\n\
warning: Skein.lean:13:5: unused variable `h`\n\
";
